 ...
 */
static int state_index_rc(const struct kscan_ec_config *config, const int row, const int col) {
...
static int kscan_ec_configure(const struct device *dev, const kscan_callback_t callback) {
...
static void kscan_ec_timer_handler(struct k_timer *timer) {
    struct kscan_ec_data *data = CONTAINER_OF(timer, struct kscan_ec_data, work_timer);
...
static int kscan_ec_init(const struct device *dev) {
    ...
    for (int i = 0; i < config->direct.len; i++) {
        gpio_pin_configure_dt(&config->direct.gpios[i].spec, GPIO_OUTPUT);
        gpio_pin_set_dt(&config->direct.gpios[i].spec, 0);
    }

    // Init mux sel
    for (int i = 0; i < config->mux_sels.len; i++) {
        gpio_pin_configure_dt(&config->mux_sels.gpios[i].spec, GPIO_OUTPUT);
    }

    // Enable mux
    gpio_pin_configure_dt(&config->mux_en.spec, GPIO_OUTPUT);
    gpio_pin_set_dt(&config->mux_en.spec, 0);
...
static const struct kscan_driver_api kscan_ec_api = {.config = kscan_ec_configure,
...
#define KSCAN_EC_INIT(n)                                                                           \
    ...
    static int kscan_ec_activity_event_handler_wrapper##n(const zmk_event_t *eh) {                 \
    ...
    ZMK_SUBSCRIPTION(kscan_ec##n, zmk_activity_state_changed);                                     \
                                                                                                   \
    DEVICE_DT_INST_DEFINE(n, &kscan_ec_init, NULL, &kscan_ec_data_##n, &kscan_ec_config_##n,       \
                          APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY, &kscan_ec_api);
